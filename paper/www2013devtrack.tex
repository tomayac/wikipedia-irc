...
As the article titles of all language versions
of the particular article's cluster are know,
we use them for cross-language searches, as can be seen in \autoref{fig:screenshot}.
...
as the search results are meant to be consumed by a~human evaluator.
Driven by (H1), we assume breaking news events to be discussed on social networks.
We will show the details of this argument in \autoref{sec:premature-evaluation},
for now, we assume social networks to be a~short period ahead of Wikipedia.
...
(``X is a'' to ``X was a'', or the addition of a~death date).

The at time of writing recent breaking news event
of the resignation of Pope Benedict XVI helps respond to (Q1).
The first three edit times of the Pope's English Wikipedia article%
\footnote{Edit history en: \url{http://en.wikipedia.org/w/index.php?title=Pope_Benedict_XVI&action=history},
accessed 02/18/2013} on February 11, 2013,
the day of his resignation, are as follows
...
for smaller events of more regional impact as well, for example,
when Indian singer Varsha Bhosle committed suicide%
\footnote{Varsha Bhosle suicide: \url{en.wikipedia.org/wiki/Varsha_Bhosle},
accessed 02/18/2013} on 8 October 2012.
...
which we have provided, however, at this point we do not have access to the system
of Osborne \emph{et~al.}

Regarding Wikipedia Live Monitor's scalability, we could scale the monitoring system
...
we limit ourselves to the Wikipedias with 100,000 articles and more
at no remarkable loss of recall.

\section{Future Work}

Future work will mainly address two areas.
First, the automatic categorization of edits on Wikipedia needs to be more fine-grained.
...
Second, the connection between social network search and Wikipedia edits
...
In this paper, we have shown an application called Wikipedia Live Monitor
